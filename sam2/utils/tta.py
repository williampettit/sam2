...
class TTAManager:
    def __init__(self, threshold: float = 0.5):
        ...
        self.pil_augmentations: List[Tuple[Callable, Callable]] = [
            ...
            (lambda img: pil_adjust_brightness(img, factor=1.5), lambda m: m),  # brightness
            (lambda img: pil_adjust_contrast(img, factor=1.5), lambda m: m),  # contrast
            (lambda img: pil_adjust_saturation(img, factor=1.5), lambda m: m),  # saturation
